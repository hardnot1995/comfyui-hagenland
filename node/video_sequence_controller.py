...
class VideoSequenceControllerNode:
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        return {
            ...
            "optional": {
                # Add a dummy input to force execution order
    ...
    def process(self, unique_id, reset_flag, starter_image, work_location_path,
                total_number_of_segments, project_name, manual_step, use_manual_step, feedback_dependency=None):
        """
        ...
        try:
            ...
            if state is None or reset_flag:
                ...
                state = {
                    ...
                    'total_segments': total_number_of_segments,
                    'current_step': 1,
                    'is_finished': False,
            ...
            # === Case 2: Auto-Restart After Finishing ===
                ...
                state = { # Rebuild state
                    ...
                    'work_folder_video': video_folder,
                    'total_segments': total_number_of_segments,
                    'current_step': 1, # Reset step
                    'is_finished': False, # Reset finished flag
                    'start_time': time.time(),
                    'last_update': time.time()
                }
            ...
            return (error_image, "output/error", "output/error", 0, True)
...
class VideoSequenceFeedbackNode:
    NODE_NAME = "Video Sequence Feedback 🔄"
    NODE_GROUP = "Video"
    CATEGORY = "🔄Hagenland"
    DESCRIPTION = "Updates the controller with the last processed frame from a video segment."
    ...
    def update_controller(self, processed_image, unique_id):
        """
        ...
        try:
            ...
            current_step = state.get('current_step', 1) # Get the step number controller is *expecting* feedback for
            ...
            # Return the processed image (as per ComfyUI node requirements)
            return (processed_image,)

        except Exception as e:
            error_msg = f"Error in update_controller: {str(e)}"
